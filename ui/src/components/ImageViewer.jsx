...
export default function ImageViewer({
    ...
    if (overlayEnabled) {
      nodes.forEach((node) => {
        ...
        ctx.fillStyle = isHovered
          ? colors.canvasLabelBgHover
          : colors.canvasLabelBg;
      ...
      const selectedNode = selectedEntityId
        ? nodes.find((n) => n.id === selectedEntityId && n.bbox)
        : null;
      if (selectedNode) {
  ...
  return (
    <div className="relative" style={{ width, height }}>
      ...
      <div className="absolute top-2 left-2 flex items-center gap-3 pointer-events-none">
        <span className="flex items-center gap-1.5">
          <span
            ...
            style={{
              borderColor: "var(--canvas-node-default)",
        ...
        <span className="flex items-center gap-1.5">
          <span
            className="w-3 h-3 rounded-sm border-2"
            style={{
              borderColor: "var(--highlight-hover)",
              backgroundColor: "var(--highlight-hover)",
              opacity: 0.6,
            }}
          />
          <span
        ...
        <span className="flex items-center gap-1.5">
          <span
            className="w-3 h-3 rounded-sm border-2"
            style={{
              borderColor: "var(--highlight-selected)",
      ...
      {/* Overlay toggle */}
      <label
        className="absolute bottom-2 left-2 flex items-center gap-1.5 cursor-pointer select-none z-10"
        style={{ color: "var(--text-secondary)" }}
      >
        <input
          type="checkbox"
      ...
      <div className="absolute bottom-2 right-2 flex items-center gap-2">
        ...
        <div className="flex flex-col gap-0.5">
          <button
            ...
            className="p-1 rounded cursor-pointer"
            style={locked ? btnActiveStyle : btnStyle}
